    ...
    jQuery.extend({
        Deferred: function () {
            ...
            jQuery.each(tuples, function (i, tuple) {
                ...
                defererd[tuple[0]] = list.fire;
                //promise.resolveWith || rejectWith || notifyWith
                defererd[tuple[0] + "With"] = list.fireWith;
            ...
            promise.promise(defererd);    //这里传递参数defered是为了将promise身上的方法扩展给到defered

            return defererd;
        },
        when: function (async) {
            //这里的async实际上就是defer,由于defer在上面方法中已经被扩展了promise方法，故这里才可以调用。
            //同时，这里不用传参数，返回的就是一个promise对象。
