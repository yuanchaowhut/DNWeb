   ...
   */
    ...
    */
    ...
    _.shuffle = function (obj) {
        var set = _.isArray(obj) ? obj : _.values(obj),   //如果obj是Object，则获取其value组成的数组
            length = set.length,
            shuffle = new Array(set.length);
        for (var index = 0, rand; index < length; index++) {
            rand = _.random(0, index);
            // console.log(rand);
            //实际上，如果不乱序，则是shuffle[index] = set[index]，乱序就加了一个rand与index交换步骤
    ...
    _.sample = function (obj, n) {
        //如果没有传n，并且obj是Object类型，则随机返回一个value回去
        if (n == null) {
            if (!_.isArray(obj)) {
                obj = _.values(obj);
            }
    ...
    //防抖函数
    _.debounce = function (func, wait, rightnow) {
        var args, timer, time;
        //处理函数(对func的包装处理)
        var later = function () {
            //获取时间戳
            var last = _.now() - time;
            if (last < wait) {
                timer = setTimeout(later, wait - last);  //当高频刷新time时，last永远小于wait，则func一直执行不了。
            } else {
                timer = null;
                if (!rightnow) {
        ...
        return function () {
            ...
            time = _.now();
            //是否立即执行处理函数
            var callNow = rightnow && !timer;
            //当高频调用时，此匿名函数高频执行，time不断刷新，但是timer在规定时间内只会赋值1次
            if (!timer) {
